...
(function ($, undefined) {
    var OFormChanger = {
        debug: false,
        device: (window.ontouchstart === undefined ? 'desktop' : 'touch'), // winWidth = parseInt($(window).width(), 10) ( winWidth > 1000 ? 'desktop' : 'touch' ),
        basePrefix: 'ch',
        themeClass: '',
        ...
        createWraps: function (hash, cls, opts) {
            ...
            for (var i = 1; i <= opts.wraps; i++) {
                if (i == 1) {
                    ...
                    var container = $("span[data-valmsg-for='" + hash.field[0].name + "']");
                    if (container && container.length && $(hash.field[0]).hasClass('form-control'))
                        wraps[i].append(container);
                    continue;
                }
    ...
    $.extend($.fn, {
        ...
        changerText: function (settings) {
            this.filter('input:text,input[type="number"], input:password, textarea').each(function () {
                if (!$(this).data().changer) init($(this));
            });

            function init(field) {
                var is_textarea = field.is('textarea'),
                    is_number = field.attr('type') == 'number',
                ...
                // input[type="number"]
                if (is_number) {
                    hash.data.wrap.addClass(cls.number);

                    hash.numbValue = $('<span/>', { 'class': 'numbValue' });
                    hash.data.wrap.append(hash.numbValue);


                    hash.arrUp = $('<span/>', { 'class': cls.arrUp, 'role': 'ArrUp' });
                    hash.arrDown = $('<span/>', { 'class': cls.arrDown, 'role': 'ArrDown' });

                    hash.data.wrap.append(hash.arrUp);
                    hash.data.wrap.append(hash.arrDown);

                    hash.arrUp.add(hash.arrDown).click(function () {
                        var is_down = $(this).is('.' + cls.arrDown),
                    ...
                    hash.arrUp.add(hash.arrDown).bind('mousedown', function () {
                        var is_down = $(this).is('.' + cls.arrDown),
                            val = parseInt(field.val(), 10) || 0;
                    ...
                    hash.arrUp.add(hash.arrDown).bind('mouseup', function () {
                ...
                hash.data.wrap.bind('change pull click mouseover mouseout focus focusin blur focusout disable enable input readonly notReadonly mousedown mouseup press unpress invalid valid chFocusout chDisable chEnable chReadonly chNotReadonly', function (e) {
                    ...
                    switch (e.type) {
                        ...
                        case 'pull':
                            ...
                            // input[type="number"]
                            if (is_number) {
                                hash.numbValue.text(hash.field.val())
                            }
        ...
        changerSelect: function (settings) {
            ...
            function init(field) {
                ...
                if (OFormChanger.device != 'touch') {
                    ...
                    //hash.listin.scrollBox();
                    if (opts.enableScroll) hash.listin.scrollBox();

                    //скрываем реальный селект и сбрасываем табиндекс, чтобы не фокусился
                    field.attr({ 'tabindex': '-1' }).css({ 'position': 'absolute', 'left': '-5000px' });
                }
                else {
                    hash.data.wrap.addClass(cls.touch);
                }
                ...
                hash.data.wrap.bind('change pull click mouseover mouseout focus focusin focusout disable enable mousedown mouseup press unpress invalid valid open close', function (e) {
                    ...
                    switch (e.type) {
                        ...
                        case 'click':
                            if ((!isList || selfOpt) && OFormChanger.device != 'touch')
                        ...
                        case 'focusout':
                            ...
                            if (e.originalEvent && ($(e.originalEvent.relatedTarget).parents('.' + cls.wrap).length || $(e.originalEvent.toElement).parents('.' + cls.wrap).length)) return false;
